...
def calibrate_gaze(
    cyclopeanPOR_XY, truePOR_XY, method=cv2.RANSAC, threshold=5, plottingFlag=False
):

    result = cv2.findHomography(
        cyclopeanPOR_XY, truePOR_XY, method=method, ransacReprojThreshold=threshold
    )
    # print(result[0])
    ...
    if plottingFlag == True:
        ...
        plt.plot(
            cyclopeanPOR_XY[frameCount, 0],
            cyclopeanPOR_XY[frameCount, 1],
            "g2",
            label="Uncalibrated POR",
        )
        plt.plot(
            truePOR_XY[frameCount, 0],
            truePOR_XY[frameCount, 1],
            "r8",
            label="Ground Truth POR",
        )
        # l1, = plt.plot([],[])
