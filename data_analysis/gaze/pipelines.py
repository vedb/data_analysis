...
def pupil_monocular_v01(session_folder, 
	...
	if os.path.exists(pupil_file_left):
		...
		inputs_pupil_left = dict(fpaths=dict(eye_video=eye_left_video_file,
				timestamps=eye_left_time_file,),
			variable_names=None)
		# Run pupil detection
		pupil_list_left = vmp.utils.batch_run(fn_pupil, inputs_pupil_left, 
	...
	if os.path.exists(pupil_file_right):
		...
		inputs_pupil_right = dict(fpaths=dict(eye_video=eye_right_video_file,
				timestamps=eye_right_time_file,),
			variable_names=None)
		# Run pupil detection
		pupil_list_right = vmp.utils.batch_run(fn_pupil, inputs_pupil_right, 
			batch_size=batch_size_pupil,
			batch_combine_fn=vmp.utils.list_reduce,
			progress_bar=progress_bar,
	...
	if os.path.exists(ref_file):
		...
		inputs_marker = dict(fpaths=dict(video_data=world_video_file,
				timestamps=world_time_file),
			variable_names=None)
	...
	# need changing.
	method, result_left = calibrate_pl.calibrate_2d_monocular(matched_data_left[0], frame_size=(video_vdim,video_hdim))
	...
	# and matched data only returns a 1-long tuple. If we want binocular, this will 
	# need changing.
	method, result_right = calibrate_pl.calibrate_2d_monocular(matched_data_right[0], frame_size=(video_vdim,video_hdim))
	# Create mapper for gaze
