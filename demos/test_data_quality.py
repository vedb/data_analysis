...
def plot_marker_images(mean_image_cal, mean_image_val, path):
    fig, axs = plt.subplots(nrows=1, ncols=2)
    fig.set_figheight(10)
    fig.set_figwidth(10)
    axs[0].set_title("Calibration Average Image", fontsize=18)
    axs[0].imshow(mean_image_cal.astype(int))
    axs[0].tick_params(axis='x', labelsize=14)
    axs[0].tick_params(axis='y', labelsize=14)
    axs[1].set_title("Validation Average Image", fontsize=18)
    axs[1].imshow(mean_image_val.astype(int))
    axs[1].tick_params(axis='x', labelsize=14)
    axs[1].tick_params(axis='y', labelsize=14)
    saving_directory = path + "/data_quality_checks"
    file_name = os.path.join(saving_directory, "average_marker_images.png")
...
def plot_average_eye_images(mean_image_0, mean_image_1, path):
    ...
    axs[0].imshow(mean_image_0.astype(int), cmap='gray')
    axs[0].tick_params(axis='x', labelsize=14)
    axs[0].tick_params(axis='y', labelsize=14)
    axs[1].set_title("Left Eye Average Image", fontsize=18)
    axs[1].imshow(mean_image_1.astype(int), cmap='gray')
    axs[1].tick_params(axis='x', labelsize=14)
    axs[1].tick_params(axis='y', labelsize=14)
    saving_directory = path + "/data_quality_checks"
...
def plot_marker_position(world_dataframe_cal, world_dataframe_val, path):
    ...
    axs[0].scatter(world_dataframe_cal["cal_norm_pos_x"], 1 - world_dataframe_cal["cal_norm_pos_y"], c="orange",
    ...
    axs[1].scatter(world_dataframe_val["val_norm_pos_x"], 1 - world_dataframe_val["val_norm_pos_y"], c="cyan",
                   edgecolors='k', s=58, alpha=0.9)
    axs[1].grid(True)
    axs[1].set_xlim([0, 1])
    axs[1].set_ylim([0, 1])
...
def plot_data_quality(my_session, pupil_dataframe, mean_image_0, mean_image_1,
                      world_dataframe_cal, mean_image_cal,
                      world_dataframe_val, mean_image_val):
    plot_average_eye_images(mean_image_0, mean_image_1, my_session.session_path)
    plot_pupil_confidence(pupil_dataframe, my_session.session_path)
    plot_all_fps(my_session)
    plot_marker_position(world_dataframe_cal, world_dataframe_val, my_session.session_path)
    plot_marker_images(mean_image_cal, mean_image_val, my_session.session_path)
...
def run_marker_detection_0(session, frame_range=(0,500), skip_frame=4, scale=0.5):
    ...
    hdim = world_frame_size[0]
    vdim = world_frame_size[1]
    a = np.zeros((int(vdim*scale), int(hdim*scale), 3), dtype=np.float64) + 120
    current_frame = copy.deepcopy(a)
    current_frame.setflags(write=1)
    ...
    for frame_index in t:
        ...
        frame_3_channel = np.stack((frame,) * 3, axis=-1)
        ...
        for iw, w in enumerate(world_circles):
            ...
            for e in ellipses:
                ...
                x = ellipse_centers.mean(0).tolist()[0]
                y = ellipse_centers.mean(0).tolist()[1]
                df.loc[count, "cal_location_x"] = x / scale
                df.loc[count, "cal_location_y"] = y / scale
                df.loc[count, "cal_norm_pos_x"] = x / (scale * hdim)
                df.loc[count, "cal_norm_pos_y"] = y / (scale * vdim)
                ...
                ref_pixel_x = int(df.loc[count, "cal_location_x"] * scale)
                ref_pixel_y = int((df.loc[count, "cal_location_y"]) * scale)
                df.loc[count, "method"] = detection_method
                img = cv2.circle(frame_3_channel, (ref_pixel_x, ref_pixel_y), 8, (200, 25, 205), 2)
                range_x = np.arange(max(0, ref_pixel_x - boundary), min(int(scale * hdim) - 1, ref_pixel_x + boundary))
                range_y = np.arange(max(0, ref_pixel_y - boundary), min(int(scale * vdim) - 1, ref_pixel_y + boundary))
                # print(range_x,range_y)
                pp = img[range_y.min():range_y.max()][:, range_x.min():range_x.max()]
                current_frame[range_y.min():range_y.max()][:, range_x.min():range_x.max()] = pp
                frame = img
                count = count + 1
...
def run_marker_detection_1(session, frame_range=(0, 500), skip_frame=4, scale=0.5):
    ...
    # TODO: termination criteria: Make inputs?
    ...
    for frame_index in t:
        ...
        if ret:
            ...
            frame_3_channel = cv2.drawChessboardCorners(frame_3_channel, (6, 8), corners, ret)
            ...
            ref_pixel_x = int(marker_position[0]*scale)
            ref_pixel_y = int(marker_position[1]*scale)
            frame_3_channel = cv2.circle(frame_3_channel,
                               (int(marker_position[0]*scale), int(marker_position[1]*scale)),
                               8, (25, 200, 205), 2)
            corners[:, 0] = corners[:, 0] * (1 / hdim)
            corners[:, 1] = corners[:, 1] * (1 / vdim)
            marker_position = np.mean(corners, axis=0)
            df.loc[count, "val_norm_pos_x"] = marker_position[0]
            df.loc[count, "val_norm_pos_y"] = marker_position[1]
            range_x = np.arange(max(0, ref_pixel_x - boundary), min(int(scale * hdim)-1, ref_pixel_x + boundary))
            range_y = np.arange(max(0, ref_pixel_y - boundary), min(int(scale * vdim)-1, ref_pixel_y + boundary))
            pp = frame_3_channel[range_y.min():range_y.max()][:, range_x.min():range_x.max()]
            current_frame[range_y.min():range_y.max()][:, range_x.min():range_x.max()] = pp
            count = count + 1
            frame = frame_3_channel
...
if __name__ == "__main__":
    ...
    scale = 0.5
    world_dataframe_cal, mean_image_cal = run_marker_detection_0(session=my_session,
                                                                 frame_range=(start_index, end_index),
                                                                 skip_frame=15, scale=scale)
    ...
    end_index = 60 * 30 * int(minutes) + 30 * int(seconds)
    scale = 0.5
    world_dataframe_val, mean_image_val = run_marker_detection_1(session=my_session,
                                                                 frame_range=(start_index, end_index),
                                                                 skip_frame=15, scale=scale)
    ...
    plot_data_quality(my_session, pupil_dataframe, mean_image_0, mean_image_1,
                      world_dataframe_cal, mean_image_cal,
                      world_dataframe_val, mean_image_val)
